...
dns_f5xc_add() {
    ...
    f5_process_errors "DEBUG dns_f5xc: Adding TXT record for $fulldomain"
    
    # Get root domain and subdomain
    if ! _get_root "$fulldomain"; then
        f5_process_errors "ERROR dns_f5xc: Could not find zone for domain: $fulldomain"
        return 1
    fi
    ...
    f5_process_errors "DEBUG dns_f5xc: Adding TXT record for $actual_subdomain in zone $domain"
    
    # Get current zone configuration
    if ! _f5xc_rest "GET" "/api/config/dns/namespaces/system/dns_zones/$domain"; then
        f5_process_errors "ERROR dns_f5xc: Failed to get zone configuration"
        return 1
    fi
    ...
    if ! _add_txt_record_to_zone "$zone_config" "$actual_subdomain" "$txtvalue"; then
        f5_process_errors "ERROR dns_f5xc: Failed to add TXT record to zone"
        return 1
    fi
    ...
    if [ $put_exit_code -eq 2 ]; then
        ...
        f5_process_errors "ERROR dns_f5xc: Duplicate TXT record detected - cannot add duplicate"
        return 1
    elif [ $put_exit_code -ne 0 ]; then
        f5_process_errors "ERROR dns_f5xc: Failed to update zone"
        return 1
    fi
    
    # Wait for DNS propagation to authoritative nameservers
    if ! _wait_for_dns_propagation "$domain" "$actual_subdomain" "$txtvalue"; then
        f5_process_errors "WARN dns_f5xc: TXT record not propagated to authoritative nameservers within timeout"
        # Continue anyway - ACME validation will fail if truly not propagated
    fi
    
    f5_process_errors "DEBUG dns_f5xc: Successfully added TXT record"
    return 0
}
...
dns_f5xc_rm() {
    ...
    if ! _get_root "$fulldomain"; then
        f5_process_errors "ERROR dns_f5xc: invalid domain"
        return 1
    fi

    f5_process_errors "DEBUG dns_f5xc: Removing TXT record for $_subdomain in zone $_domain"
    ...
    if ! _remove_txt_record_from_zone "$response" "$_subdomain" "$txtvalue"; then
        f5_process_errors "DEBUG dns_f5xc: No TXT record found to remove for $_subdomain"
        return 0
    fi
    ...
    if [ $put_exit_code -eq 2 ]; then
        ...
        f5_process_errors "ERROR dns_f5xc: Unexpected duplicate TXT record error during removal"
        return 1
    elif [ $put_exit_code -ne 0 ]; then
        f5_process_errors "ERROR dns_f5xc: Failed to update zone configuration"
        return 1
    fi

    f5_process_errors "DEBUG dns_f5xc: Successfully removed TXT record"
    return 0
}
...
_get_root() {
    ...
    f5_process_errors "DEBUG dns_f5xc: Finding root zone for: $fulldomain"
    
    # Split domain into parts and try to find the zone
    domain="$fulldomain"
    
    while [ -n "$domain" ]; do
        f5_process_errors "DEBUG dns_f5xc: Checking zone: $domain"
        ...
        if _f5xc_rest "GET" "/api/config/dns/namespaces/$namespace/dns_zones/$domain" 2>/dev/null; then
            ...
            if echo "$response" | grep -q '"name":\s*"'"$domain"'"' || echo "$response" | grep -q '"name": "'"$domain"'"'; then
                f5_process_errors "DEBUG dns_f5xc: Zone found via API: $domain"
                ...
                f5_process_errors "DEBUG dns_f5xc: Subdomain: $subdomain"
        ...
        if [ "$old_domain" = "$domain" ]; then
            f5_process_errors "DEBUG dns_f5xc: Domain unchanged, breaking loop: $domain"
            break
        fi
    done
    
    f5_process_errors "DEBUG dns_f5xc: No zone found for: $fulldomain"
    return 1
}
...
_add_txt_record_to_zone() {
    ...
    f5_process_errors "DEBUG dns_f5xc: RRSet name: $rrset_name"
...
_parse_and_modify_zone() {
    ...
    if command -v jq >/dev/null 2>&1; then
        f5_process_errors "DEBUG dns_f5xc: Using jq for zone modification"
        if ! _modify_zone_with_jq "$zone_config" "$subdomain" "$txt_value" "$rrset_name" "$action"; then
            return 1
        fi
    else
        f5_process_errors "ERROR dns_f5xc: jq is required but not available"
        return 1
    fi
...
_modify_zone_with_jq() {
    ...
    if [ "$action" = "add" ]; then
        ...
        if [ $? -ne 0 ]; then
            f5_process_errors "ERROR dns_f5xc: jq processing failed"
            return 1
        fi
    else
        # Remove TXT record from zone
        f5_process_errors "DEBUG dns_f5xc: Removing TXT record"
        ...
        if [ $? -ne 0 ]; then
            f5_process_errors "ERROR dns_f5xc: jq removal processing failed"
            return 1
        fi
    fi
...
_validate_config() {
    f5_process_errors "DEBUG dns_f5xc: Validating F5 XC configuration files"
    
    # Check required environment variables
    if [ -z "$F5XC_TENANT" ]; then
        f5_process_errors "ERROR dns_f5xc: F5XC_TENANT is required"
        return 1
    fi
    
    # Check if we have either certificates or API token
    if [ -z "$F5XC_CLIENT_CERT" ] && [ -z "$F5XC_API_TOKEN" ]; then
        f5_process_errors "ERROR dns_f5xc: Either F5XC_CLIENT_CERT or F5XC_API_TOKEN is required"
        return 1
    fi
    ...
    if [ -n "$F5XC_CLIENT_CERT" ]; then
        if [ ! -f "$F5XC_CLIENT_CERT" ]; then
            f5_process_errors "ERROR dns_f5xc: Client certificate file not found: $F5XC_CLIENT_CERT"
            return 1
        fi
        
        if [ ! -r "$F5XC_CLIENT_CERT" ]; then
            f5_process_errors "ERROR dns_f5xc: Client certificate file not readable: $F5XC_CLIENT_CERT"
            return 1
        fi
        ...
        if [ "$F5XC_CLIENT_CERT" != "${F5XC_CLIENT_CERT%.p12}" ] || [ "$F5XC_CLIENT_CERT" != "${F5XC_CLIENT_CERT%.pfx}" ]; then
            if [ -z "$F5XC_CERT_PASSWORD" ]; then
                f5_process_errors "ERROR dns_f5xc: Certificate password (F5XC_CERT_PASSWORD) is required for P12 certificates"
                return 1
            fi
        fi
        
        f5_process_errors "DEBUG dns_f5xc: Client certificate file validated"
    elif [ -n "$F5XC_API_TOKEN" ]; then
        f5_process_errors "DEBUG dns_f5xc: Using API token authentication"
    fi
    
    f5_process_errors "DEBUG dns_f5xc: Configuration validated successfully"
    return 0
}
...
_f5xc_rest() {
    ...
    f5_process_errors "DEBUG dns_f5xc: API call: $method $path"
    
    # Check authentication method: Client certificates (preferred) or API token (fallback)
    if [ -n "$F5XC_CLIENT_CERT" ]; then
        # Use client certificate authentication
        f5_process_errors "DEBUG dns_f5xc: Using client certificate authentication"
        ...
        if [ "$F5XC_CLIENT_CERT" != "${F5XC_CLIENT_CERT%.p12}" ] || [ "$F5XC_CLIENT_CERT" != "${F5XC_CLIENT_CERT%.pfx}" ]; then
            ...
            if [ $? -ne 0 ]; then
                f5_process_errors "ERROR dns_f5xc: Failed to convert P12 certificate to PEM format"
                return 1
            fi
            f5_process_errors "DEBUG dns_f5xc: Using PEM certificate"
        fi
        ...
        if [ "$method" = "POST" ] || [ "$method" = "PUT" ]; then
            f5_process_errors "DEBUG dns_f5xc: Request data included"
            curl_cmd="$curl_cmd -d '$data'"
        fi
        
        f5_process_errors "DEBUG dns_k5xc: Executing curl command"
        f5_process_errors "DEBUG dns_f5xc: Full curl command: $curl_cmd"
        ...
        if [ "$cert_file" != "$F5XC_CLIENT_CERT" ]; then
            rm -f "$cert_file"
            f5_process_errors "DEBUG dns_f5xc: Cleaned up temporary PEM file"
        fi
        
    elif [ -n "$F5XC_API_TOKEN" ]; then
        # Use API token authentication with direct curl
        f5_process_errors "DEBUG dns_f5xc: Using API token authentication with direct curl"
        ...
        f5_process_errors "DEBUG dns_f5xc: Executing curl command"
        response=$(eval "$curl_cmd")
        curl_exit_code=$?
    else
        f5_process_errors "ERROR dns_f5xc: No valid authentication method found - credentials not properly validated"
        return 1
    fi
    
    # Check curl exit code
    if [ "$curl_exit_code" != "0" ]; then
        f5_process_errors "ERROR dns_f5xc: curl error for $method $full_url (exit code: $curl_exit_code)"
        return 1
    fi
    
    f5_process_errors "DEBUG dns_f5xc: API response received"
    ...
    if echo "$response" | grep -q '"code":[0-9]'; then
        ...
        if [ "$error_code" != "0" ] && [ "$error_code" != "null" ] && [ "$error_code" != "unknown" ]; then
            f5_process_errors "ERROR dns_f5xc: F5 XC API error (code: $error_code): $error_message"
            
            # Check for specific error types
            if echo "$response" | grep -q "duplicate.*TXT"; then
                f5_process_errors "ERROR dns_f5xc: Duplicate TXT record detected - this is not allowed in F5 XC"
...
_wait_for_dns_propagation() {
    ...
    f5_process_errors "DEBUG dns_f5xc: Waiting for TXT propagation to nameservers: $nameservers"
    ...
    while [ $attempt -le $max_attempts ]; do
        f5_process_errors "DEBUG dns_f5xc: Propagation check attempt $attempt/$max_attempts (delay: ${delay}s)"
        ...
        for ns in $nameservers; do
            ...
            if command -v dig >/dev/null 2>&1; then
                ...
                f5_process_errors "WARN dns_f5xc: No DNS query tools available"
                return 1
            fi
            ...
            if echo "$result" | grep -q "$expected_value"; then
                ...
                f5_process_errors "DEBUG dns_f5xc: TXT record found on $ns: $result"
            else
                f5_process_errors "DEBUG dns_f5xc: TXT record not yet on $ns (got: $result)"
            fi
        done
        
        # Success if propagated to all nameservers
        if [ $propagated -eq $total_ns ] && [ $total_ns -gt 0 ]; then
            f5_process_errors "DEBUG dns_f5xc: TXT record propagated to all $total_ns nameservers"
            return 0
        fi
        
        # Wait with exponential backoff
        if [ $attempt -lt $max_attempts ]; then
            f5_process_errors "DEBUG dns_f5xc: Waiting ${delay}s before next check..."
            sleep $delay
            delay=$((delay * 2))
            if [ $delay -gt 30 ]; then
    ...
    f5_process_errors "WARN dns_f5xc: TXT record propagation timeout after $max_attempts attempts"
    return 1
}
